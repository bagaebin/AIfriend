...
3. `needNewImage`가 true이면 OpenAI 이미지 모델을 호출하고 공개 URL을 저장/발급합니다.
4. 서버가 `{ reply, profile, imageUrl, needNewImage }`로 응답합니다.
5. 클라이언트는 reply를 채팅에 추가하고, `needNewImage`가 true일 때 아바타 텍스처를 교체하며, 웹캠 기반 표정으로 프레임마다 메쉬 변형을 유지합니다.

## 성능 및 복원력
- **OpenAI 호출:** 렌더 루프 밖에서 이미지 생성을 처리하고, 프로필이 크게 바뀔 때만 호출을 제한합니다.
